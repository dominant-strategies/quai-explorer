...
const transactions = require('../graphql/transactions.js')
...
export const mutations = {
  setInitialTransactions(state, initialTransactions) {
    state.txData = initialTransactions
  },
  ...
  setTransactionsValues(state, data) {
    ...
    state.transactionsGraphData[data.index][0].data.push({
      x: data.x,
      y: data.y,
    })
  },
...
export const actions = {
  async fetchTransaction({ commit, rootState }) {
    const data = await axios.post(
      rootState.api,
      {
        query: transactions.GET_TRANSACTIONS,
        variables: {
          num: 10,
        },
      },
    ...
    commit('setInitialTransactions', data.data.data.transactions)
  },
  async fetchTx({ commit, dispatch }, payload) {
    ...
    payload = {
      ...payload,
      txCount: parseInt(txNum.data.result, 16),
    }
    ...
    try {
      var tx = await axios.post(
        'http://45.32.69.88/' + payload.chain + '-http',
        {
          ...
          params: [
            '0x' + payload.data.number[payload.position].toString(16),
            true,
          ],
          id: 1,
        },
