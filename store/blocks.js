...
export const mutations = {
    ...
    addBlockTableData(state, payload){
      ...
      const getTotal = async () => {
        const getTotalTransactions = await getTotalTx()
        return getTotalTransactions.result
      } 
     
      console.log(getTotal())
      // parseInt(res.result, 16)
      ...
      state.blocksData.unshift(newRowBlocks)
    }
}
...
export const actions = {
    ...
    createSockets({ commit }) {
      ...
      const handleConnection = (connection, chain, position) => {
        ...
        connection.onopen = () => {
          console.log('Successfully connected to the echo websocket server of', chain)
        ...
        connection.onmessage = (event) => {
          ...
          if (data.params != undefined) {
            ...
            commit('addBlockTableData', payload)
          }
        }
